# Quantum Computing Science Association

> *The future of computing is quantum. Be part of building it.*

The **Quantum Computing Science Association (QCSA)** is a student-driven nonprofit uniting the brightest minds across Southern California universities to explore, learn, and push the boundaries of quantum computing: one campus at a time.

---

## Why Quantum? Why Now?

Quantum computing is no longer science fiction. It is reshaping cryptography, drug discovery, optimization, and artificial intelligence at a pace the world has never seen. The students who understand it today will be the researchers, engineers, and founders who define tomorrow.

QCSA exists to make sure you are one of them.

---

## What We Offer

- **Workshops & Bootcamps**: Hands-on sessions covering quantum circuits, algorithms (Grover's, Shor's, VQE, and more), and platforms like Qiskit, Cirq, and PennyLane
- **Research Opportunities**: Collaborate with peers and faculty on cutting-edge quantum projects
- **Quantum Device Consortium (QDC)**: A dedicated space for builders: hackathons, open-source projects, and developer resources at [qdc-qcsa.org](https://qdc-qcsa.org)
- **Industry Connections**: Network with professionals and organizations at the forefront of the quantum industry
- **Cross-Campus Collaboration**: Work alongside students from UCLA, USC, Caltech, UCSD, and UCSB in a growing Southern California quantum network

Whether you are a complete beginner or already deep in quantum theory, there is a place for you here.

---

## Our Chapters

QCSA currently oversees five university chapters across Southern California:

| Branch | Status | Links |
|--------|--------|-------|
| **UCLA** *(founding chapter)* | Active | [qcsa-ucla.org](https://qcsa-ucla.org) · [qdc-qcsa.org](https://qdc-qcsa.org) |
| **USC** | Coming soon | — |
| **Caltech** | Coming soon | — |
| **UCSD** | Coming soon | — |
| **UCSB** | Coming soon | — |

---

## Get Involved

Are you a student, researcher, or enthusiast who wants to be part of the quantum revolution?

- **Join a chapter** — Connect with the UCLA chapter now at [qcsa-ucla.org](https://qcsa-ucla.org)
- **Start or lead a chapter** — Interested in bringing QCSA to your campus? We are actively expanding
- **Contribute** — Browse our repositories, open issues, and submit pull requests to help build the tools and resources that power our community

---

## About

QCSA is a registered nonprofit organization founded at UCLA. Our mission is to democratize access to quantum computing education and foster a collaborative, inclusive community of the next generation of quantum scientists and engineers across university campuses.

The nonprofit website is currently in development. Stay tuned for updates.
